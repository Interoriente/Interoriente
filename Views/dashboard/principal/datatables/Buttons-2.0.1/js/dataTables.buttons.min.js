...
(function (e) {
  ...
  e.extend(v.prototype, {
      ...
      if (b !== p) {
        ...
        for (c = 0; c < b.length; c++)
          this._expandButton(
            a.buttons,
            b[c],
            b[c] !== p && b[c].config !== p && b[c].config.split !== p,
            !0,
    ...
    _expandButton: function (a, b, c, d, l, f, k) {
      ...
      for (var r = q.length; c < r; c++) {
        ...
        if (m)
          if (
            ((b = m.config !== p && m.config.split ? !0 : !1), Array.isArray(m))
          )
            this._expandButton(
              a,
              m,
            ...
            var g = this._buildButton(
              m,
              d,
              m.split !== p || (m.config !== p && m.config.split !== p),
              l
            );
            if (g) {
                ...
                if (g.conf.split)
                  for (var t = 0; t < g.conf.split.length; t++)
                    "object" === typeof g.conf.split[t] &&
                      ...
                      g.conf.split[t].collectionLayout === p &&
                        (g.conf.split[t].collectionLayout =
                          g.conf.collectionLayout),
                      g.conf.split[t].dropup === p &&
                        (g.conf.split[t].dropup = g.conf.dropup),
                      g.conf.split[t].fade === p &&
                        (g.conf.split[t].fade = g.conf.fade));
                else
                  e(g.node).append(
                    e(
      ...
      if ("absolute" === d) {
        ...
        if (
          ...
          k.hasClass("dt-btn-split-wrapper") &&
            ...
            ((t = k.children("button.dt-btn-split-drop").offset().left),
            (g = k.children("button.dt-btn-split-drop").outerWidth()),
            (x = t + g));
          g = 0;
          if (c.hasClass(f.rightAlignClassName))
            (g = m - r),
              d > q + g &&
                ((d -= q + g), (n -= r + g), (g = d > n ? g + n : g + d));
      ...
      var c = [],
        d = function (h, n, q) {
          for (var r, m, g = 0, t = n.length; g < t; g++)
            if ((r = n[g]))
              (m = q !== p ? q + g : g + ""),
                h.push({ node: r.node, name: r.conf.name, idx: m }),
        ...
        l = function (h, n) {
          ...
          if (Array.isArray(h) || h instanceof e)
            ...
            if (-1 !== h.indexOf(","))
              for (r = h.split(","), m = 0, q = r.length; m < q; m++)
                l(r[m].trim(), n);
            else if (h.match(/^\d+(\-\d+)*$/))
              ...
              for (h = h.replace(":name", ""), m = 0, q = r.length; m < q; m++)
                r[m].name === h && c.push({ inst: n, node: r[m].node });
            else
              e(m)
                .filter(h)
                .each(function () {
  ...
  var K = function (a) {
    ...
    Q = function (a, b) {
      ...
      f = f.nodes().toArray();
      for (
        var k = b.length, h = [], n = 0, q = 0, r = 0 < k ? l.length / k : 0;
        q < r;
        q++
      ) {
